...
add_thickbox();
$wimpie_lite_recommended_plugins = array(
	'contact-form-7' => array( 'recommended' => true ),
	...
	<div class="feature-section recommended-plugins">
		<?php foreach ( $wimpie_lite_recommended_plugins as $plugin => $prop ) { ?>
		<?php
		...
		switch ( $active['needs'] ) {
			...
			$label = __( 'Install', 'wimpie-lite' );
			break;
			case 'activate':
			$class = 'activate-now button button-primary';
			$label = __( 'Activate', 'wimpie-lite' );
			break;
			case 'deactivate':
			$class = 'deactivate-now button';
			$label = __( 'Deactivate', 'wimpie-lite' );
			break;
		}
		...
		<div class="col plugin_box">
			<?php if ( $prop['recommended'] ): ?>
				<span class="recommended"><?php _e( 'Required', 'wimpie-lite' ); ?></span>
			<?php endif; ?>
			...
			<div class="b-wrap">
			<span class="version"><?php echo __( 'Version:', 'wimpie-lite' ); ?><?php echo esc_html( $info->version ) ?></span>
